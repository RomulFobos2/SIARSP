...
public class RequestForDeliveryService {
    ...
    @Transactional
    public boolean approveByDirector(Long id, String commentText, Employee currentEmployee) {
        return process(id, commentText, currentEmployee, RequestStatus.PENDING_ACCOUNTANT);
    }

    @Transactional
    public boolean rejectByDirector(Long id, String commentText, Employee currentEmployee) {
        return process(id, commentText, currentEmployee, RequestStatus.REJECTED_BY_DIRECTOR);
    }
    ...
    private boolean process(Long id, String commentText, Employee currentEmployee, RequestStatus targetStatus) {
        ...
        if (!workflowValidator.canTransition(request.getStatus(), targetStatus, "ROLE_EMPLOYEE_MANAGER")) {
            log.warn("Согласование директором заявки id={} отклонено", id);
            return false;
        }
