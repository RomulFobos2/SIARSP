...
public class Supply {
     ...
     * Копируется из RequestedProduct при приёмке поставки
     *
     * Примеры: шт., кг, л, уп., коробка
     */
    @Column(length = 100)
    private String unit;
    ...
    /**
     * Количество недопоставленного товара (дефицит)
     *
     * Разница между заказанным количеством (из RequestedProduct) и фактически принятым (quantity).
     * Значение 0 означает, что весь товар принят без замечаний.
     *
     * Примеры:
     * - Заказали 200 шт., получили 200 шт. → deficitQuantity = 0 (полная поставка)
     * - Заказали 200 шт., получили 195 шт. → deficitQuantity = 5 (недопоставка)
     * - Заказали 100 шт., получили 97 шт.  → deficitQuantity = 3 (брак/повреждение)
     *
     * Используется для:
     * - Определения статуса заявки (RECEIVED vs PARTIALLY_RECEIVED)
     * - Анализа надёжности поставщика
     * - Формирования отчётов по недопоставкам
     */
    @Column(nullable = false)
    private int deficitQuantity = 0;

    /**
     * Причина дефицита (недопоставки)
     *
     * Текстовое описание причины, по которой товар был недопоставлен.
     * Обязательно для заполнения, если deficitQuantity > 0.
     *
     * Примеры:
     * - "2 шт. с повреждённой упаковкой"
     * - "5 шт. не доставлено поставщиком"
     * - "3 шт. истёк срок годности"
     */
    @Column(length = 500)
