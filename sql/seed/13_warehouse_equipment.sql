INSERT INTO t_warehouse_equipment (
    name,
    serial_number,
    production_date,
    useful_life_years,
...
('Погрузчик Toyota 8FG-1', 'FLT-0001', '2022-03-10', 8,
 (SELECT id FROM t_equipment_type WHERE name = 'Погрузчик'), 'IN_USE',
...
('Холодильная камера Polair KXH-01', 'REF-1001', '2021-01-15', 10,
 (SELECT id FROM t_equipment_type WHERE name = 'Холодильная камера'), 'IN_USE',
 (SELECT id FROM t_warehouse WHERE name = 'Холодильный склад')),
