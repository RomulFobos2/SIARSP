...
-- ВНИМАНИЕ: «Срок годности» теперь хранится как ЧИСЛО ДНЕЙ хранения партии,
-- а не как дата. Конкретные даты годности живут в Supply (productionDate + expirationDate).
...
FROM (
    ...
    SELECT 'МЛК-001', 'Срок годности', '365' UNION ALL
    ...
    SELECT 'МЛК-002', 'Срок годности', '7' UNION ALL
    ...
    SELECT 'СЫР-002', 'Срок годности', '90' UNION ALL
    SELECT 'СЫР-002', 'Массовая доля жира', '50' UNION ALL
    SELECT 'СЫР-002', 'Срок созревания', '3' UNION ALL

    -- === Говядина тушёная ГОСТ 338г (ГВД-001, REGULAR) — консервы, длительное хранение ===
    ...
    SELECT 'ГВД-001', 'Срок годности', '1825' UNION ALL
    SELECT 'ГВД-001', 'Сорт мяса', 'Высший' UNION ALL
    SELECT 'ГВД-001', 'Масса нетто', '0.338' UNION ALL

    -- === Говядина охлаждённая вырезка 1кг (ГВД-002, REFRIGERATOR) — скоропорт ===
    ...
    -- === Курица тушёная ГОСТ 325г (КРЦ-001, REGULAR) — консервы ===
    ...
    -- === Филе куриное охлаждённое 0.8кг (КРЦ-002, REFRIGERATOR) — скоропорт ===
    ...
    SELECT 'ЯБЛ-002', 'Срок годности', '60' UNION ALL
    ...
    SELECT 'ОГУ-002', 'Срок годности', '14' UNION ALL
